class SecureWallet {
    ...
    async deleteMetaMaskAccount() {
        // Placeholder - performing a deletion depends on how you manage accounts (server/Firebase/localStorage)
    ...
    updateMetaMaskUI(isConnected) {
        ...
        if (disconnectBtn) {
            // Avoid duplicate listeners — remove previous then add
    ...
    // Use either window.firebaseDB (if set by you) or global firebase if loaded
    getFirebaseDB() {
  ...
  async checkEmailAttempts(email) {
    ...
    const ref = firebase.firestore().collection('email_verification').doc(email);
    ...
    if (!data.emailAttempts || (data.emailLockout && now > data.emailLockout)) {
      await ref.set({ emailAttempts: 1, emailLockout: null }, { merge: true });
      this.showToast('Email verified successfully!', 'success');
  this.switchScreen('dashboard-screen');
      return;
    }
    if (data.emailAttempts >= 3) {
      const lockoutPeriod = 2 * 60 * 60 * 1000;
      await ref.set({ emailLockout: now + lockoutPeriod }, { merge: true });
      throw new Error('Too many attempts. Try after a few hours.');
    }
    await ref.set({ emailAttempts: data.emailAttempts + 1 }, { merge: true });
  }

  // Ensure this is at the start of sendOTP()
  generateOTP() {
  ...
  if (userLoaded && this.userData.passwordHash && this.userData.seedPhrase && this.userData.walletAddress) {
    ...
    // New or incomplete user, show password setup
    this.showToast("Email verified successfully!", "success");
...
showOTPSection() {
  // Example: Show or unhide the OTP section in your UI
  ...
  // ...rest of your SecureWallet class

    resendOTP = async () => {
        console.log('Resending OTP...');
        // hide resend section and clear timers
...
// Or display a new password setup, etc.
    ...
    validatePassword() {
        ...
        if (!passwordInput || !confirmPasswordInput || !continueBtn) {
            // Elements may not exist if not on that screen
            return;
        }
...
SecureWallet Team`;
    ...
    tryQRGeneration(canvas, address, currency, amount, nonce) {
        ...
        if (typeof QRCode !== 'undefined' && QRCode.toCanvas) {
            try {
                QRCode.toCanvas(canvas, qrData, { width: 256, height: 256, margin: 2 }, (error) => {
                    if (error) {
    ...
    createTransactionItem(tx, isRecent = false) {
        ...
        // Use onclick that references global "app" instance
        return `
